...
public class FinishingOperationService(LabelsMisDbContext db, ICurrentUserService currentUser)
{
    ...
    public async Task<FinishingOperation> CreateAsync(FinishingOperationForm form, CancellationToken ct = default)
    {
        ...
        var operation = FinishingOperation.Create(Guid.NewGuid(), form.Code, form.Description, form.OperationType,
            form.DefaultSetupMinutes, form.DefaultRunSpeedFpm, form.EquipmentName, form.CostPerHour, userId, DateTime.UtcNow);
    ...
    public async Task UpdateAsync(Guid id, FinishingOperationForm form, CancellationToken ct = default)
    {
        ...
        operation.Update(form.Code, form.Description, form.OperationType, form.DefaultSetupMinutes,
            form.DefaultRunSpeedFpm, form.EquipmentName, form.CostPerHour, RequireUserId(), DateTime.UtcNow);
        await db.SaveChangesAsync(ct);
    }
